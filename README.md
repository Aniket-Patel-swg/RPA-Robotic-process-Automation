...
The Client layer is responsible for interacting with external websites, handling their response data, and managing errors. it contains service clients and REST clients.
   ## 1) Service Client
   It acts as an adapter and calls the REST client, which will handle the actual interaction. The service client manages the logic for calling REST clients according to the requirement and also handles handling.
      * Calls REST client. 
   ...
   REST Client executes RESTful API calls to external systems.
      ...
      * return response from an external website to Service Client.
